...
namespace sg {
	...
	private:
        ...
        void BindTextureRGBA(GLuint texture, int width, int height, unsigned char* data) {
    ...
    public:
        ...
        GLuint SetTextureRGBA(const char* filename) {
            int width, height, nrChannels;
            unsigned char* data = stbi_load(filename, &width, &height, &nrChannels, 0);
            GLuint texture;
            if (data) {
                glGenTextures(1, &texture);
                BindTextureRGBA(texture, width, height, data);
            }
            else {
                printf("Failed to load texture %s\n", filename);
            }
        ...
        void SetTexturesData(sg::Material* mat) {
            if (mat->texture_Kd.isPresent && !mat->texture_Kd.isLoaded) {
                ...
                if (!wasPresent) {
                    index = SetTextureRGBA(mat->texture_Kd.map);
                }
            ...
            if (mat->texture_Ks.isPresent && !mat->texture_Ks.isLoaded) {
                ...
                bool wasPresent = index != -1;
                if (!wasPresent) {
                    index = SetTextureRGBA(mat->texture_Ks.map);
                }
            ...
            if (mat->texture_Ka.isPresent && !mat->texture_Ka.isLoaded) {
                ...
                bool wasPresent = index != -1;
                if (!wasPresent) {
                    index = SetTextureRGBA(mat->texture_Ka.map);
                }
