...
vec3 CalcDirLightComponent(vec3 albedo, vec3 specular, vec3 camDir) {
	...
	vec3 p = dirLightViewPosition.xyz;
	p.z *= 0.99;
	p /= dirLightViewPosition.w;
	if (p.x > 1 || p.x < 0 || p.y > 1 || p.y < 0 || p.z > 1.0 || p.z < 0.0) {
		diffuseComponent = 0; specularComponent = 0;
	} else {
		float litValue = texture(dirLight.shadowTexture, p);
		diffuseComponent *= litValue;
		specularComponent *= litValue;
	}
	
	// blinn-phong
	vec3 shading = dirLight.color * (diffuseComponent * albedo) + specular * specularComponent;
	return dirLight.intensity * shading;
}
...
void main() {
	...
	vec3 shading = vec3(0.);
	shading += CalcSpotLightComponent(albedo, specular, camDir);
	shading += CalcDirLightComponent(albedo, specular, camDir);
